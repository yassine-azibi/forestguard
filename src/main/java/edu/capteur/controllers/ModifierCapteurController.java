...
public class ModifierCapteurController {
    ...
    @FXML private ComboBox<String> cbLocalisation;
    @FXML private ComboBox<String> cbType, cbStatut;
    @FXML private ComboBox<Foret>  cbForet;
    @FXML private Label lblErreurNom, lblErreurLocalisation, lblErreurType,
                        lblErreurStatut, lblErreurForet;
    ...
    public void initialize() {
        ...
        cbForet.valueProperty().addListener((obs, old, foret) -> {
            if (foret != null) {
                String locActuelle = getLocalisation();
                if (locActuelle.isEmpty()) cbLocalisation.setValue(foret.getLocalisation());
            }
            clearErrorCombo(lblErreurForet, cbForet);
        });

        tfNom.textProperty().addListener((obs, old, val) -> clearError(lblErreurNom, tfNom));
        cbLocalisation.valueProperty().addListener((obs, old, val) -> clearErrorCombo(lblErreurLocalisation, cbLocalisation));
        cbLocalisation.getEditor().textProperty().addListener((obs, old, val) -> clearErrorCombo(lblErreurLocalisation, cbLocalisation));
        cbType.valueProperty().addListener((obs, old, val) -> clearErrorCombo(lblErreurType, cbType));
    ...
    private List<String> extraireLocalisations(List<Foret> forets) {
        ...
        for (Foret f : forets) {
            if (f.getLocalisation() != null && !f.getLocalisation().isBlank()) {
                ...
                if (!toutesRegions.contains(loc)) toutesRegions.add(loc);
            }
        }
    ...
    private String getLocalisation() {
        ...
        if (val == null || val.isBlank())
            val = cbLocalisation.getValue() != null ? cbLocalisation.getValue() : "";
        return val.trim();
    }
    ...
    public void setCapteur(Capteur c) {
        ...
        cbLocalisation.setValue(null);
        cbLocalisation.getEditor().clear();

        if (c.getForetId() > 0) {
            cbForet.getItems().stream()
                    .filter(f -> f.getId() == c.getForetId())
                    .findFirst()
                    .ifPresent(cbForet::setValue);
        }
        if (cbForet.getValue() == null && cbForet.getItems().size() == 1)
            cbForet.setValue(cbForet.getItems().get(0));
    }
    ...
    public void modifier() {
        ...
        capteur.setNom(tfNom.getText().trim());
        capteur.setType(cbType.getValue());
        capteur.setLocalisation(getLocalisation());
        capteur.setStatut(cbStatut.getValue());
        capteur.setForetId(foretSelectionnee.getId());
        capteurService.modifier(capteur);
        fermer();
    }
    ...
    private boolean valider() {
        ...
        Validator.ResultatValidation resultat = Validator.validerCapteur(
                tfNom.getText(), getLocalisation(), cbType.getValue(), cbStatut.getValue());
    ...
    private void setError(Label label, TextField field, String message) {
        if (label != null) { label.setText(message); label.setTextFill(Color.RED); label.setVisible(true); label.setManaged(true); }
